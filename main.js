...
case 'ytmp300': {
    ...
    try {
        ...
        const {
            title, thumbnail, fduration, views, channel, publish, id,
            data: audioData
        } = data;
...
⏳ *Procesando tu música...*`;
        ...
        const filePath = path.join(tmpDir, `${Date.now()}_music.mp3`);

        const audioRes = await axios.get(audioData.url, {
            responseType: 'stream',
