    ...
    try {
        ...
        if (rpgData.usuarios[userId]?.cooldowns?.batallaMascota) {
            ...
            if ((Date.now() - cooldownTime) < 5 * 60 * 1000) {
                ...
                return sock.sendMessage(msg.key.remoteJid, { text: `⏳ *Debes esperar ${remainingTime} segundos antes de usar .batallamascota nuevamente.*` }, { quoted: msg });
            }
        }
        ...
        // 📌 Verificar si se mencionó o citó a un usuario
        ...
        if (!opponentId) {
            return sock.sendMessage(msg.key.remoteJid, { text: "⚔️ *Menciona o responde a un usuario para retarlo a una batalla de mascotas.*" }, { quoted: msg });
        }

        // 📌 Verificar si el oponente tiene mascota
        ...
        // 🔥 Enviar reto de batalla con info de mascotas y habilidades
        let habilidadesUser = userMascot.habilidades.map(h => `⚡ *${h.nombre}:* Nivel ${h.nivel}`).join("\n");
        let habilidadesOpponent = opponentMascot.habilidades.map(h => `⚡ *${h.nombre}:* Nivel ${h.nivel}`).join("\n");

        await sock.sendMessage(msg.key.remoteJid, {
            text: `⚔️ *¡Desafío de Batalla de Mascotas!*\n\n
👤 *Retador:* @${userId.split('@')[0]}  
🎯 *Retado:* @${opponentId.split('@')[0]}  

🐾 *Mascota de @${userId.split('@')[0]}:*  
- 🏷️ *Nombre:* ${userMascot.nombre}  
...
- 🏷️ *Nombre:* ${opponentMascot.nombre}  
...
⏳ *Tienes 2 minutos para aceptar antes de que la solicitud expire.*`,
        ...
        setTimeout(() => {
            if (rpgData.usuarios[userId].battleRequest && rpgData.usuarios[userId].battleRequest.target === opponentId) {
                delete rpgData.usuarios[userId].battleRequest;
                fs.writeFileSync(rpgFile, JSON.stringify(rpgData, null, 2));
                sock.sendMessage(msg.key.remoteJid, { text: "⏳ *La solicitud de batalla ha expirado porque no fue aceptada a tiempo.*" }, { quoted: msg });
            }
        }, 120000);
    } catch (error) {
...
case 'gomascota': {
    try {
        ...
        if (!challengerId) {
            return sock.sendMessage(msg.key.remoteJid, { text: "⚠️ *No tienes ninguna solicitud de batalla pendiente.*" }, { quoted: msg });
        }

        // 📌 Verificar si la solicitud sigue activa
        const requestTime = rpgData.usuarios[challengerId].battleRequest.time;
        if (Date.now() - requestTime > 120000) { // 2 minutos
            delete rpgData.usuarios[challengerId].battleRequest;
            fs.writeFileSync(rpgFile, JSON.stringify(rpgData, null, 2));
            return sock.sendMessage(msg.key.remoteJid, { text: "⏳ *La solicitud de batalla ha expirado.*" }, { quoted: msg });
        }
        ...
        const animaciones = [
            ...
            `🌪️ *Ambas mascotas se enfrentan con fuerza... ¡es una batalla intensa!*`,
            `✨ *Los espectadores están al borde de sus asientos... ¿quién ganará?*`,
            "💥 *¡Impacto final! La batalla está por decidirse...*"
        ];

        let mensajeAnimado = await sock.sendMessage(msg.key.remoteJid, { text: animaciones[0] }, { quoted: msg });
        ...
        // **💥 Cálculo de batalla**
        const statsChallenger = challengerMascot.nivel * 5 + challengerMascot.habilidades.reduce((total, h) => total + h.nivel * 2, 0);
        const statsUser = userMascot.nivel * 5 + userMascot.habilidades.reduce((total, h) => total + h.nivel * 2, 0);
        ...
        if (statsChallenger > statsUser) {
            ...
            return sock.sendMessage(msg.key.remoteJid, { text: "🤝 *¡La batalla terminó en empate!*" });
        }
        ...
        ganadorMascota.experiencia += xpGanador;
        rpgData.usuarios[ganadorId].coins = (rpgData.usuarios[ganadorId].coins || 0) + coinsGanador;
        perdedorMascota.experiencia += xpPerdedor;
        rpgData.usuarios[perdedorId].coins = (rpgData.usuarios[perdedorId].coins || 0) + coinsPerdedor;
...
- ${perdedorMascota.nombre}: ${perdedorMascota.vida} HP`;
        ...
        // ⏳ Guardar cooldown de batalla
        rpgData.usuarios[ganadorId].cooldowns = rpgData.usuarios[ganadorId].cooldowns || {};
        rpgData.usuarios[perdedorId].cooldowns = rpgData.usuarios[perdedorId].cooldowns || {};
        rpgData.usuarios[ganadorId].cooldowns.batallaMascota = Date.now();
        rpgData.usuarios[perdedorId].cooldowns.batallaMascota = Date.now();
        ...
        return sock.sendMessage(msg.key.remoteJid, { text: '❌ *Error inesperado al procesar la batalla.*' }, { quoted: msg });
    }
    break;
}
...
case 'minar2': {
    try {
        ...
        if (usuario.cooldowns?.minar2 && tiempoActual - usuario.cooldowns.minar2 < cooldownTime) {
            ...
            return sock.sendMessage(msg.key.remoteJid, { text: `⏳ *Debes esperar ${tiempoRestante} minutos antes de volver a minar.*` }, { quoted: msg });
        }

        // 🎖️ **Generar recompensas aleatorias**
        let xpGanado = Math.floor(Math.random() * (2000 - 500 + 1)) + 500; // Entre 500 y 2000 XP
        let diamantesGanados = Math.floor(Math.random() * (900 - 300 + 1)) + 300; // Entre 300 y 900 diamantes

        // ❤️ Reducir vida aleatoriamente entre 3 y 7 puntos
...
✨ *${xpGanado} XP obtenidos*`
        ...
        // 📊 **Habilidad aleatoria que sube de nivel (30% de probabilidad)**
...
case 'crime': {
    try {
        ...
        if (usuario.cooldowns?.crime && tiempoActual - usuario.cooldowns.crime < cooldownTime) {
            ...
            return sock.sendMessage(msg.key.remoteJid, { text: `⏳ *Debes esperar ${tiempoRestante} minutos antes de intentar otro crimen.*` }, { quoted: msg });
        }

        // 🎖️ **Generar recompensas aleatorias**
        let exito = Math.random() < 0.5;
        let xpGanado = exito ? Math.floor(Math.random() * (2000 - 500 + 1)) + 500 : -Math.floor(Math.random() * (1000 - 300 + 1)) + 300;
        let diamantesGanados = exito ? Math.floor(Math.random() * (1000 - 200 + 1)) + 200 : 0;
...
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
            `🚗 *Un golpe rápido y preciso, ${usuario.nombre} robó un auto de lujo y lo vendió en el mercado negro.*  
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
...
case 'work': {
    try {
        ...
        if (usuario.cooldowns?.work && tiempoActual - usuario.cooldowns.work < cooldownTime) {
            ...
            return sock.sendMessage(msg.key.remoteJid, { text: `⏳ *Debes esperar ${tiempoRestante} minutos antes de volver a trabajar.*` }, { quoted: msg });
        }

        // 🎖️ **Generar recompensas aleatorias**
        let xpGanado = Math.floor(Math.random() * (3500 - 200 + 1)) + 200;
        let diamantesGanados = Math.floor(Math.random() * (1000 - 200 + 1)) + 200;
...
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
            `💰 *${usuario.nombre} completó una tarea importante en su trabajo y fue recompensado.*  
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
...
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
            `🚀 *Un ascenso inesperado hizo que ${usuario.nombre} ganara más dinero del esperado.*  
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`
        ];
        ...
        const rangos = [
            { nivel: 1, rango: "🌟 Novato" },
            { nivel: 5, rango: "⚔️ Guerrero Novato" },
            { nivel: 10, rango: "🔥 Maestro del Oficio" },
            { nivel: 20, rango: "👑 Experto en Trabajo" },
            { nivel: 30, rango: "🌀 Líder de Proyecto" },
            { nivel: 40, rango: "💀 Empresario Exitoso" },
            { nivel: 50, rango: "🚀 Magnate Supremo" }
        ];
        let rangoAnterior = usuario.rango;
        usuario.rango = rangos.reduce((acc, curr) => (usuario.nivel >= curr.nivel ? curr.rango : acc), usuario.rango);
...
case 'minar': {
    try {
        ...
        // 🎖️ **Generar recompensas aleatorias**
        let xpGanado = Math.floor(Math.random() * (2000 - 200 + 1)) + 200;
...
✨ *${xpGanado} XP ganados*`
        ...
        // 🌟 **Incrementar niveles aleatorios en habilidades (50% de probabilidad)**
        let habilidades = Object.keys(usuario.habilidades);
        if (habilidades.length > 0 && Math.random() < 0.5) { // 50% de probabilidad
            let habilidadSubida = habilidades[Math.floor(Math.random() * habilidades.length)];
            usuario.habilidades[habilidadSubida].nivel += 1;
        ...
        const rangos = [
            { nivel: 1, rango: "🌟 Novato" },
            { nivel: 5, rango: "⚔️ Guerrero Novato" },
            { nivel: 10, rango: "🔥 Maestro Minero" },
            { nivel: 20, rango: "👑 Excavador Supremo" },
            { nivel: 30, rango: "🌀 Cazador de Gemas" },
            { nivel: 40, rango: "💀 Señor de los Minerales" },
            { nivel: 50, rango: "🚀 Maestro de las Minas" }
        ];
        let rangoAnterior = usuario.rango;
        usuario.rango = rangos.reduce((acc, curr) => (usuario.nivel >= curr.nivel ? curr.rango : acc), usuario.rango);
...
case 'picar': {
    try {
        ...
        if (usuario.cooldowns?.picar && tiempoActual - usuario.cooldowns.picar < cooldownTime) {
            ...
            return sock.sendMessage(msg.key.remoteJid, { text: `⏳ *Debes esperar ${tiempoRestante} minutos antes de volver a picar.*` }, { quoted: msg });
        }
...
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
            `💎 *Tras una larga jornada, ${usuario.nombre} encontró gemas valiosas.*  
💎 *${diamantesGanados} Diamantes obtenidos*  
✨ *${xpGanado} XP ganados*`,
...
✨ *${xpGanado} XP ganados*`
        ...
        await sock.sendMessage(msg.key.remoteJid, { text: "❌ *Ocurrió un error al picar. Inténtalo de nuevo.*" }, { quoted: msg });
    }
    break;
}                        
