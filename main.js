...
// Función para generar un código de 8 dígitos
function generateSubBotCode() {
    return crypto.randomBytes(4).toString("hex").toUpperCase();
}

// Cargar subbots desde el archivo
function loadSubBots() {
    return JSON.parse(fs.readFileSync(subbotsFile, "utf-8"));
}

// Guardar subbots en el archivo
...
case "modoprivado": {
    ...
    if (!subbots[senderId]) {
        return sock.sendMessage(msg.key.remoteJid, { text: "❌ *No eres un sub-bot registrado.* Usa .serbot primero." }, { quoted: msg });
    }

    subbots[senderId].mode = "privado";
    saveSubBots(subbots);

    return sock.sendMessage(msg.key.remoteJid, { text: "🔒 *Modo privado activado.* Solo tú puedes usar este bot." }, { quoted: msg });
}
break;
...
case "modopublico": {
    ...
    subbots[senderId].mode = "publico";
    saveSubBots(subbots);

    return sock.sendMessage(msg.key.remoteJid, { text: "🌍 *Modo público activado.* Cualquier usuario podrá interactuar con este bot." }, { quoted: msg });
}
break;

case "subot": {
    let subbots = loadSubBots();
    let subBotsList = Object.keys(subbots);

    if (subBotsList.length === 0) {
        return sock.sendMessage(msg.key.remoteJid, { text: "🚫 *No hay sub-bots conectados.*" }, { quoted: msg });
    }

    let message = "🤖 *Lista de Sub-Bots Activos:*\n\n";
    subBotsList.forEach((bot, index) => {
