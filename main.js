...
if (!args.length || !/^https?:\/\/(www\.)?(youtube\.com|youtu\.be)/.test(args[0])) { return; } await sock.sendMessage(msg.key.remoteJid, { react: { text: '⏳', key: msg.key } }); const videoUrl = args[0]; try { const result = await savetube.ytdlaud(videoUrl); if (!result.status || !result.response || !result.response.descarga) { throw new Error(); } const videoInfo = result.response; const tmpDir = path.join(__dirname, 'tmp'); if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir); const filePath = path.join(tmpDir, `${Date.now()}.mp3`); const response = await fetch(videoInfo.descarga); if (!response.ok) throw new Error(); const buffer = await response.buffer(); fs.writeFileSync(filePath, buffer); const fileSize = fs.statSync(filePath).size; if (fileSize < 10000) { fs.unlinkSync(filePath); throw new Error(); } await sock.sendMessage(msg.key.remoteJid, { audio: fs.readFileSync(filePath), mimetype: 'audio/mpeg', fileName: `${videoInfo.titulo}.mp3` }, { quoted: msg }); fs.unlinkSync(filePath); await sock.sendMessage(msg.key.remoteJid, { react: { text: '✅', key: msg.key } }); } catch { await sock.sendMessage(msg.key.remoteJid, { react: { text: '❌', key: msg.key } }); } break; 
}
